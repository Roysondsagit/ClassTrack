...
export default function MarkAttendance() {
    ...
    const handleSubmit = async () => {
        ...
        if (Object.keys(attendance).length === 0) { setError('Please mark attendance for at least one student.'); return }
        ...
        const { error } = await supabase
            .from('attendance')
            .upsert(records, { onConflict: 'user_id,class_id,date' })

        if (error) setError(error.message)
        else {
            setSuccess(`✅ Attendance saved for ${records.length} student(s) on ${date}!`)
            setAttendance({})
        }
        setLoading(false)
    }
                ...
                <div className="table-wrapper">
                    <div className="table-header">
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                            ...
                            <button className="btn btn-sm btn-ghost" onClick={() => setAttendance({})}>
                                🔄 Clear
                            </button>
                        </div>
                    </div>
                        ...
                        users.map(u => (
                            <div className="attendance-row" key={u.id}>
                                ...
                                <div className="attendance-btns">
                                    <button
                                        className={`att-btn present ${attendance[u.id] === 'present' ? 'active' : ''}`}
                                        onClick={() => toggleAttendance(u.id, 'present')}
                                    >✅ Present</button>
                                    <button
                                        className={`att-btn absent ${attendance[u.id] === 'absent' ? 'active' : ''}`}
                                        onClick={() => toggleAttendance(u.id, 'absent')}
                                    >❌ Absent</button>
                                </div>
                            </div>
                        ))
